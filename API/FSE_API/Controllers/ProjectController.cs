            ...
            try
            {
                var project = repository.Get().Select(b =>
                    new ProjectModel()
                    {
                        ...
                        CompletedTask = b.Tasks.Count == 0 ? 0 :Math.Round( ((double)b.Tasks.Count(i => i.Status == "C")) / b.Tasks.Count * 100)
                    });
        ...
        public HttpResponseMessage GetProject(int i)
        {
            try
            {
                ...
                var projectdto = new ProjectModel()
                {
                    ...
                    CompletedTask = project.Tasks.Count == 0 ? 0 : Math.Round(((double)project.Tasks.Count(c =>c.Status == "C")) / project.Tasks.Count * 100)
                };
